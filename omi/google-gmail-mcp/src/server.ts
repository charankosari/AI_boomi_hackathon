...
async function initClients() {
  try {
    ...
    gmailClient = google.gmail({ version: "v1", auth: auth as any });
...
initClients().then((success) => {
  if (!success) {
    console.error(
      "Failed to initialize Google API clients. Server will not work correctly."
    );
  } else {
...
function decodeEmailBody(message: gmail_v1.Schema$Message): string {
  let body = "";
  if (message.payload) {
...
server.tool(
  "list-emails",
  {
    query: z
      ...
      .optional()
      .describe(
        "Gmail search query (e.g., 'is:spam', 'is:unread', 'from:example@gmail.com')"
      ),
    maxResults: z
      .number()
      .optional()
    ...
    try {
      ...
      if (messages.length === 0) {
        ...
        for (const message of messages.slice(0, maxResults)) {
          ...
          content += `From: ${from}\n`;
          content += `Date: ${date}\n`;
          content += `Body: ${body.substring(0, 200)}${
            body.length > 200 ? "..." : ""
          }\n\n`;
        }
      }
...
server.tool(
  "delete-draft",
  {
    draftIds: z
      ...
      .describe(
        "The ID(s) of the draft(s) to delete. Can be a single ID, an array of IDs, or a comma-separated string of IDs"
      ),
  },
  async ({ draftIds }) => {
    try {
...
server.tool(
  "send-draft",
  {
    ...
    to: z
      ...
      .optional()
      .describe(
        "Optional recipient email address. If provided, will override the draft's recipient before sending"
      ),
  },
  async ({ draftId, to }) => {
    try {
      ...
      if (to) {
        ...
        if (message.raw) {
          ...
          rawMessage = Buffer.from(
            message.raw.replace(/-/g, "+").replace(/_/g, "/"),
            "base64"
          ).toString("utf-8");
        } else {
          // Build from payload
...
server.tool(
  "mark-as-spam",
  {
    messageIds: z
      .union([z.string(), z.array(z.string())])
      .describe(
        "The ID(s) of the email(s) to mark as spam. Can be a single ID, an array of IDs, or a comma-separated string of IDs"
      ),
  },
  async ({ messageIds }) => {
    try {
...
server.tool(
  "mark-as-important",
  {
    messageIds: z
      .union([z.string(), z.array(z.string())])
      .describe(
        "The ID(s) of the email(s) to mark as important. Can be a single ID, an array of IDs, or a comma-separated string of IDs"
      ),
  },
  async ({ messageIds }) => {
    try {
...
server.tool(
  "mark-as-read",
  {
    messageIds: z
      .union([z.string(), z.array(z.string())])
      .describe(
        "The ID(s) of the email(s) to mark as read. Can be a single ID, an array of IDs, or a comma-separated string of IDs"
      ),
  },
  async ({ messageIds }) => {
    try {
