...
server.resource("list-docs", "googledocs://list", async (uri) => {
  try {
...
server.tool(
  "create-doc",
  {
    ...
    content: z
      .string()
      .optional()
      .describe("Optional initial content for the document"),
  },
  async ({ title, content = "" }) => {
    try {
...
server.tool(
  "update-doc",
  {
    ...
    replaceAll: z
      .boolean()
      .optional()
      .describe("Whether to replace all content (true) or append (false)"),
  },
  async ({ docId, content, replaceAll = false }) => {
    try {
      ...
      if (replaceAll) {
        ...
        // Get the actual document end index from the API response
        // Google Docs API provides the endIndex in the document structure
        let endIndex = 1;
        if (
          doc.data.body &&
          ...
          if (lastElement.endIndex) {
            ...
            // Fallback: calculate manually but exclude the final newline
        ...
        // Delete all content (excluding the final newline/paragraph break)
        // The endIndex is exclusive, so we use endIndex - 1 to avoid the final newline
        ...
        // Insert at endIndex - 1 (one position before the end, since endIndex is exclusive)
        // This ensures we insert before the final newline/paragraph break
...
server.prompt(
  "create-doc-template",
  {
    ...
    subject: z
      .string()
      .describe("The subject/topic the document should be about"),
    style: z
      .string()
      .describe("The writing style (e.g., formal, casual, academic)"),
  },
  ({ title, subject, style }) => ({
    messages: [
      {
