...
let slidesClient: slides_v1.Slides;
...
async function initClients() {
  try {
    ...
    slidesClient = google.slides({ version: "v1", auth: auth as any });
...
server.resource(
  "get-presentation",
  new ResourceTemplate("googleslides://{presentationId}", { list: undefined }),
  async (uri, { presentationId }) => {
    try {
...
server.tool(
  "update-presentation",
  {
    ...
    slideIndex: z
      ...
      .optional()
      .describe(
        "The slide index to add content to (0-based). If not provided, adds a new slide"
      ),
  },
  async ({ presentationId, content, slideIndex }) => {
    try {
      ...
      if (
        ...
        const newSlideId =
          createSlideResponse.data.replies?.[0]?.createSlide?.objectId;
        if (!newSlideId) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Presentation updated successfully!\nPresentation ID: ${presentationId}\nContent added to slide ${
              slideIndex !== undefined ? slideIndex + 1 : slides.length + 1
            }`,
          },
        ],
      };
